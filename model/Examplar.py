...
class Inpainter:
    ...
    def update_contours(self):
        ...
        #     plt.scatter(pt[1], pt[0], c='r', s=0.3)
        
        # plt.savefig("countor.jpg")
    ...
    def get_patch_difference(self, target_patch, dst_patch):
        # TODO 
        # cv.matchTemplete(优化)
        # patch_img1 = self.get_data(self.fill_image, target_patch)
        # patch_img2 = self.get_data(self.fill_image, dst_patch)
        # cmp_patch = self.get_data(self.fill_range, target_patch)
        # plt.subplot(221), plt.imshow(patch_img1)
        # plt.subplot(222), plt.imshow(patch_img2)
        # plt.subplot(223), plt.imshow(cmp_patch)
        # plt.show()

        # find ssd here
        patch_h = target_patch[0, 1] - target_patch[0, 0]
        patch_w = target_patch[1, 1] - target_patch[1, 0]
        patch_mask = (1-self.get_data(self.fill_range, target_patch)).reshape(patch_h, patch_w, 1).repeat(3, axis=2)
        target_data = self.get_data(self.fill_image, target_patch).astype("float64") * patch_mask
        dst_data = self.get_data(self.fill_image, dst_patch).astype("float64") * patch_mask
        ...
        for i in tqdm(range(self.h-patch_h+1)):
            for j in range(self.w-patch_w+1):
                ...
                if(self.get_data(self.fill_range, dst_patch).sum() == 0):
                    ...
                    patch_diff_dist = patch_diff+patch_dist
                    if least_dis > patch_diff_dist:
                        least_dis = patch_diff_dist
    ...
    def exe_inpaint(self, filename:str, approx = False, step = 1000):
        ...
        dir = r"D:\Courses_2022_Fall\ECE4513\Projects\src\MyCode\gif\\"
        cnt = 0
        total_time = 0
        ...
        # while (total-self.fill_range.sum()).astype(int) in tqdm(range(ip_area.astype(int))):
            ...
            cost = end-start
            total_time += cost
            print("{} points left to fill, use {} seconds".format(self.fill_range.sum(), cost))
            # if(self.show):
            ...
            # self.red_square(self.get_patch(target_point))
            # plt.show()
            # plt.savefig(fname=dir + str(cnt) + ".jpg")
            # plt.cla()

            cnt += 1
        print("Exemplar inpaint end. use {} seconds".format(total_time))
        plt.imshow(self.fill_image)
        plt.show()
        # save

        out = cv2.cvtColor(self.fill_image, cv2.COLOR_BGR2RGB)

        cv2.imwrite("half.jpg", out)
        # cv2.imwrite(filename+"mask.jpg", self.fill_range)
        return self.fill_image, self.fill_range
