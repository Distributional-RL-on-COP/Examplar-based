...
def get_square_mask(img, width = 200, height = 160):
    ...
    mask_black = 255*np.ones(img.shape)
    x1,y1 = ginput(1)[0]
    x1,y1 = int(x1), int(y1)
    plt.show()
    ...
    mask_img[y1 -height//2:y1+height//2, x1-width//2:x1+width//2] = zeros
    mask_black[y1 -height//2:y1+height//2, x1-width//2:x1+width//2] = zeros
